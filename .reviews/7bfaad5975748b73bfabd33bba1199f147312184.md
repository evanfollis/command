# Review: 7bfaad5 — Close critical Next RCE and sharp libheif advisories
...
`images: { unoptimized: true }` in `next.config.js` removes the `sharp` attack surface from the served build. Dependency overrides are belt-and-suspenders. No logic changes.
