# Review: 9b2eeda — Re-pin evaluator toolchain to the verified current binaries
...
Multi-hash update. All hashes updated consistently. Note: `NODE_SHA256` pins v22.22.0 (eval harness) while `build-production.sh` uses v24.18.0 (production build). The toolchain split is intentional but undocumented in pin comments; future maintainers may try to reconcile and break one pin. No blocking defect.
