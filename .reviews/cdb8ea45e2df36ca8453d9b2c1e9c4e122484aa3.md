# Review: cdb8ea4 — Remove the last raw evaluator runtime read
...
`evalTelemetry.ts` `listLatestEvalRuns` now reads only from `.prompteval` baselines, not from runtime report files. The read path simplification is correct and removes a dependency on runtime-owned paths. No blocking defect.
