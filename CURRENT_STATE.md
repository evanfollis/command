...
**Last updated**: 2026-07-17T06-20-00Z — tick. Prompt fixes committed (a56cb6c) for 2 must-pass prompteval failures identified in today's 04:23/05:12 UTC runs. review-prompt baseline PASSED (1.0) at 05:36 UTC. Three baseline runs now in progress as detached systemd scope (prompteval-baselines.scope, PIDs 1869852/1869853/1869854). Logs at /opt/workspace/runtime/.meta/prompteval-*.log. Deploy still blocked pending baseline completion. Observatory posture fix (c4bb659) undeployed 3+ days.
...
- **ADR-0039 baseline runs: infrastructure complete, runs in progress (ACTIVE — awaiting completion)**: All repair work done. Today (2026-07-17): review-prompt PASSED (1.0 aggregate, baseline.json written). thread-opening-frame and offline-synthesis-prompt had their first-run today and FAILED — both on must-pass cases. thread-opening-frame: must-pass holdout gc-45128a2d178513a7 failed because the prompt only covered git irreversibility, not filesystem deletions. offline-synthesis-prompt: must-pass gc-863e35027ce505e8 failed because the model proposed centralizing a working drift-gate implementation instead of replicating the pattern. Both prompt issues fixed (a56cb6c): thread-opening-frame now explicitly covers rm/find-delete/truncate on non-versioned paths; offline-synthesis-prompt clarifies "replicate candidate" means naming other projects, NOT moving/consolidating the existing implementation. Runs restarted in detached systemd scope (prompteval-baselines.scope) at 06:06 UTC. Also note: ~20-50 min per prompt (NOT 2-3 hours) when rate limits absent. Logs: /opt/workspace/runtime/.meta/prompteval-{offline-synthesis-prompt,thread-opening-frame,codex-task-prompt}-20260717T060644Z.log. The timing estimate in prior CURRENT_STATE ("13 min/case", "3-4 hours") was wrong — today's review-prompt run (16 cases) completed in ~21 min.
...
- **ADR-0039: baselines in progress — 1 done, 3 running (ACTIVE)**: review-prompt PASSED at 05:36 UTC 2026-07-17 (1.0, `.prompteval/review-prompt/baseline.json` written). thread-opening-frame and offline-synthesis-prompt both failed today on their first real runs (must-pass holdout cases). thread-opening-frame holdout gc-45128a2d178513a7: model provided a ready-to-run find-delete command without checking irreversibility. offline-synthesis-prompt gc-863e35027ce505e8: model proposed moving a working drift-gate implementation instead of replicating the pattern. Both prompts fixed (a56cb6c, committed 2026-07-17) and re-runs started as detached systemd scope. Logs at /opt/workspace/runtime/.meta/prompteval-{offline-synthesis-prompt,thread-opening-frame,codex-task-prompt}-20260717T060644Z.log. Run `prompteval check .` to check current state. **If runs not active (`pgrep -la python3 | grep prompteval` returns nothing), restart with:**
  ```
  ...
  ```
  Each run ~20-50 min (NOT 3-4 hours). Can run all 3 in parallel. After all 3 pass: `sed -i 's/"enforce": false/"enforce": true/' .prompteval/inventory.json`, commit, push, `npm run deploy`. Note: thread-opening-frame will still have 6 non-must-pass "defers-reversible-work-to-user" failures (eval design limitation — model identifies missing tools in eval context; not must-pass, gate passes regardless).
...
1. This file.
2. **Run `prompteval check .`** — as of 06:06 UTC 2026-07-17: 3 FAILs (offline-synthesis-prompt, codex-task-prompt, thread-opening-frame). review-prompt PASSED. Baseline runs started as detached systemd scope — check `pgrep -la python3 | grep prompteval` to see if still running. Logs at `/opt/workspace/runtime/.meta/prompteval-*-20260717T060644Z.log`. If runs completed, check `.prompteval/*/baseline.json` for each. After all 3 pass: flip `enforce: true` in `.prompteval/inventory.json`, commit, push, `npm run deploy`. Runs take ~20-50 min each, NOT 3-4 hours — that estimate was stale.
