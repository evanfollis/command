...
**Last updated:** 2026-07-19 — product boundary corrected to an authenticated, read-only owner observatory. Implementation is not deployed; root will deploy only after eval gates.
...
- `codex-task-prompt` has no accepted baseline. A root-owned release evaluation was already in flight when this work began, but it predates the product-boundary case correction and cannot be accepted for the new active golden hash.
...
- Current `prompteval check .` fails only for the absent Codex-task baseline; review also reports one unpromoted candidate warning.
