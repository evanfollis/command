...
**Last updated**: 2026-07-14T13-07-39Z — prompt-eval adoption tick. Commits this window: d701216 (fix thread-opening-frame holdout case ID mismatch), 0fde551 + 620296f (review artifacts for c3ce041 and d701216, fixed pattern-scanner false positive), f1da6e5 (README.md — newly required by preflight-deploy.sh). All pushed. Deploy blocked ONLY by prompteval baselines. Background baseline run started for offline-synthesis-prompt at 13:15 UTC, building partial cache (~3 cases in 40 min before timeout). Remaining blockers: 4 prompteval baseline runs needed (each ~2-3h). PREVIOUS CRITICAL NOW RESOLVED: `scripts/release.sh` now calls `preflight-deploy.sh` (5bea245). Three untracked files (evals API, EvalTelemetryPanel, evalTelemetry) still uncommitted — out of scope per prior session note.
...
- **ADR-0039: 5 prompteval FAILs — 4 baselines needed, deploy blocked (ACTIVE)**: `prompteval check .` returns 5 FAILs: no baseline for `thread-opening-frame`, `codex-task-prompt`, `offline-synthesis-prompt`; `review-prompt` baseline stale (spec/executor/adapter drifted after bc71c3c) and not a release run. `enforce: false` correctly set. Deploy is blocked because `preflight-deploy.sh` checks prompteval regardless of `enforce`. Repair work is complete (see "What bit" section). Remaining work: attended session running `prompteval run --id <id> --release --update-baseline --yes --allow-cached-baseline .` × 4. **Exact commands for next session:**
  ...
  ```
  Use `--allow-cached-baseline` to reuse partial cache from background runs (13:15-13:55 UTC on 2026-07-14 built ~3 cases for offline-synthesis-prompt). Expect scores to be non-perfect. Then: `sed -i 's/"enforce": false/"enforce": true/' .prompteval/inventory.json`, commit, push, and `npm run deploy`.
...
1. This file.
2. **Run `prompteval check .`** — still returns 5 FAILs (infrastructure repaired, baselines still needed). Use the exact commands above under "ADR-0039" in Known broken. Use `--allow-cached-baseline` NOT `--no-cache` so partial cache from 2026-07-14 background run is reused. After all 4 pass: flip `enforce: true` in `.prompteval/inventory.json`, commit, push, run `npm run deploy`. Each baseline run takes ~2-3 hours at observed LLM response rates — **plan for dedicated time, not a tick**.
