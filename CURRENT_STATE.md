...
**Last updated**: 2026-05-12T~19:05Z — symphony smoke now self-evicts stale `running` tasks; project-status no longer logs `fatal: not a git repository` to journal.
...
- **Login double-submission (option A unblocked, 20+ days)**: Meta-scan filter (10 lines, no blocking decision) still unimplemented. Approaching won't-fix threshold — reflection loop is now spending more attention on this than the fix would take.
...
- **Symphony task store accumulation (live failure mode)**: `runtime/symphony/tasks.json` has no eviction policy. 8 tasks in store: 2 orphaned `ready` (from smoke runs that failed before reaching the cleanup step), 1 stale `review` (acceptance demo `479c8834`, stale since 2026-05-01). A single orphaned `running` task blocks the entire per-project concurrency cap — this caused a 422 failure in the current smoke run (2026-05-12). Agent manually advanced the stuck task (`dde7fadf`) to `done` via curl. Fix needed: either (A) smoke drains its own orphaned `running` tasks before creating new ones (`scripts/smoke.ts`), or (B) add DELETE for terminal-state tasks.
...
- **Login double-submission (reframed 2026-04-25T15:48Z, 20+ days)**: class-mismatch confirmed — client-component fix rejected. Real options: (A) meta-scan filter, (B) server-side dedup window. Counter-handoff sent to general. Not approaching URGENT — users authenticate successfully. See Known Broken section for analysis. **Verdict pending**: ship option A or record won't-fix. 20+ days of carry-forward exceeds the cost of the fix.
- **Symphony task store eviction (new, 2026-05-12)**: no DELETE endpoint, no TTL. Orphaned tasks will block per-project concurrency cap. Fix: either smoke drains its own orphaned running tasks first, or add DELETE for terminal-state tasks. See Known Broken section.
