...
- Fresh release eval attempts: before the stable implementation commit, `thread-opening-frame` was denied; after commit `91a3eb4`, the first requested serialized run, `review-prompt`, was also denied before process creation. Both used `--release --yes --no-cache --update-baseline`. The environment approval layer rejected external subscription-CLI prompt/eval data transfer despite the dispatch's explicit principal authorization and expressly prohibited indirect execution or circumvention, so the remaining same-class runs were not invoked through another route. No provider call, run artifact, cache acceptance, or baseline mutation occurred. `prompteval check .` therefore remains fail-closed on `codex-task-prompt`, `offline-synthesis-prompt`, and `thread-opening-frame`; review-prompt remains the sole accepted fresh baseline.
...
- **Prompteval golden-set synthetic-majority decay (ACTIVE — 10-16 cycles)**: As of 2026-07-19T14:23Z telemetry: `thread-opening-frame` at **17 consecutive cycles**, `codex-task-prompt`/`offline-synthesis-prompt`/`review-prompt` at **11 cycles**. ADR-0039 requires acting on these — promote production interactions once baselines exist. This is a chicken-and-egg with the baseline blocker. Escalations are being emitted; consumption path is broken. As of 2026-07-19T14:23Z — no new baseline runs in this window.
- **ADR-0039 baseline runs: silently failed at 06:06 UTC (ACTIVE — needs restart)**: review-prompt PASSED (1.0 aggregate, baseline.json written, 05:36 UTC). thread-opening-frame and offline-synthesis-prompt prompt issues fixed (a56cb6c) after must-pass holdout failures. Three re-runs started 06:06 UTC via `systemd-run --scope` — **failed IMMEDIATELY** (zero new run artifacts; no prompteval call made; root cause: PATH not inherited in scope). Cache counts cited previously (263/112/35) were from pre-06:06 runs; those counts are not fresh. Restart must use attended foreground session. Each run ~20-50 min.
...
- **ADR-0039: 1 done, 3 STALLED — scope runs failed immediately (ACTIVE — needs restart)**: review-prompt PASSED at 05:36 UTC 2026-07-17 (1.0, `.prompteval/review-prompt/baseline.json` written). Prompts fixed (a56cb6c) for thread-opening-frame and offline-synthesis-prompt after must-pass holdout failures. Three re-runs started 06:06 UTC 2026-07-17 via `systemd-run --scope` — **failed IMMEDIATELY** (logs contain only start echo; zero new run artifacts produced; no prompteval call was made). Root cause: `prompteval` not in PATH inside scope — it lives in `/opt/workspace/supervisor/scripts/` which was not inherited. DO NOT retry via `systemd-run --scope`. Restart must use an attended foreground session with full PATH. Restart with:
  ```
  ...
  ```
  Each run ~20-50 min with warm cache. Run all 3 in parallel. Watch for actual completion (not just "Starting" in logs). After all 3 pass: `sed -i 's/"enforce": false/"enforce": true/' .prompteval/inventory.json`, commit, push, `npm run deploy`. Note: thread-opening-frame will still have 6 non-must-pass "defers-reversible-work-to-user" failures (eval design limitation; not must-pass, gate passes regardless).
- **Deploy gap: multiple commits not live (ACTIVE)**: Everything from 479bd4c (2026-07-12) through f1da6e5 (2026-07-14, README) is committed but not deployed. Blocked on prompteval baselines. Observatory posture fix (c4bb659) has been undeployed since 2026-07-14 — now **7+ days of wrong posture data for principal** (undeployed since 2026-07-14; as of 2026-07-19T14:23Z). `HEAD_ONLY=1 npm run deploy` is the decoupling path (does not touch prompt-governed code; prompteval gate will fail but that is expected and acceptable for this specific commit). Will fully unblock once baselines pass and preflight goes green.
...
1. This file.
2. **Run `prompteval check .`** — as of 2026-07-19T14:23Z — status unchanged: 3 FAILs (offline-synthesis-prompt, codex-task-prompt, thread-opening-frame). review-prompt PASSED. **Baseline re-runs silently failed** — `pgrep -la python3 | grep prompteval` returns nothing, no baseline.json written, logs at `/opt/workspace/runtime/.meta/prompteval-*-20260717T060644Z.log` contain only start timestamps. Cache is warm (see Known broken section). Restart the three runs manually (commands in Known broken section). After all 3 pass: flip `enforce: true` in `.prompteval/inventory.json`, commit, push, `npm run deploy`.
...
## Open carry-forwards
- **ADR-0039 adoption (ACTIVE — 3 of 4 baselines in progress)**: review-prompt DONE (1.0). thread-opening-frame and offline-synthesis-prompt: prompts fixed (a56cb6c) after today's must-pass failures, baseline runs in progress. codex-task-prompt: first run in progress. After completion: flip `enforce: true` in `inventory.json`, commit, push, `npm run deploy`. Completion report to `runtime/.handoff/general-command-prompt-eval-complete-<iso>.md`.
