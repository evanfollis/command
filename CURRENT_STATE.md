...
**Last updated**: 2026-07-18T14-23-46Z — reflection pass. Second quiet window. No attended session. Observatory posture fix (c4bb659) undeployed 5th day. Prompteval: 3 of 4 baselines still pending (scope runs confirmed broken, scope approach must not be retried). Golden-set decay: thread-opening-frame at 15 cycles, others at 9. No URGENT handoff filed (9th cycle structural gap). Deploy blocked.
...
- **Prompteval golden-set synthetic-majority decay (ACTIVE — 9-15 cycles)**: As of 2026-07-18T14:23Z telemetry: `thread-opening-frame` at **15 consecutive cycles**, `codex-task-prompt`/`offline-synthesis-prompt`/`review-prompt` at **9 cycles**. ADR-0039 requires acting on these — promote production interactions once baselines exist. This is a chicken-and-egg with the baseline blocker. Escalations are being emitted; consumption path is broken.
...
- **Deploy gap: multiple commits not live (ACTIVE)**: Everything from 479bd4c (2026-07-12) through f1da6e5 (2026-07-14, README) is committed but not deployed. Blocked on prompteval baselines. Observatory posture fix (c4bb659) has been undeployed since 2026-07-14 — now **4+ days of wrong posture data for principal**. `HEAD_ONLY=1 npm run deploy` is the decoupling path (does not touch prompt-governed code; prompteval gate will fail but that is expected and acceptable for this specific commit). Will fully unblock once baselines pass and preflight goes green.
...
1. This file.
2. **Run `prompteval check .`** — as of 14:22Z 2026-07-17: 3 FAILs (offline-synthesis-prompt, codex-task-prompt, thread-opening-frame). review-prompt PASSED. **Baseline re-runs silently failed** — `pgrep -la python3 | grep prompteval` returns nothing, no baseline.json written, logs at `/opt/workspace/runtime/.meta/prompteval-*-20260717T060644Z.log` contain only start timestamps. Cache is warm (see Known broken section). Restart the three runs manually (commands in Known broken section). After all 3 pass: flip `enforce: true` in `.prompteval/inventory.json`, commit, push, `npm run deploy`.
