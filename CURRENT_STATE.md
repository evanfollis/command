...
**Last updated**: 2026-07-14T02-23-23Z — reflection pass. Two commits in window: bc71c3c (wire 4 prompts under ADR-0039, fix template bug, clean probe, reseal holdouts) and 690a2d9 (downgrade write-tool cases to advisory, extend adapter timeout 300→380s). `enforce: false` correctly set with honest note. `prompteval check .` returns 5 FAILs — 3 no baseline, review-prompt baseline stale after bc71c3c, none ran with --release. Tick hit Claude session rate limit again at ~2am UTC before baseline completed — structural timing collision (2nd consecutive window). CRITICAL NEW FINDING: `scripts/release.sh` does NOT call `preflight-deploy.sh` — deploy gate is NOT enforced by the deploy chain. Three untracked files (evals API, EvalTelemetryPanel, evalTelemetry) uncommitted for 4th window. Observatory still unreviewed (3rd cycle).
...
- **ADR-0039 baseline runs blocked by tick rate-limit timing (2nd window, ACTIVE)**: Two consecutive tick sessions (b1a13a3f 2026-07-13, 166d477e 2026-07-14) hit Claude session rate limit at ~2am UTC before baseline runs completed. `enforce: false` is correctly set (false certification from prior window corrected in bc71c3c). `prompteval check .` returns 5 FAILs. Baselines must be run in an attended session or before 00:50 UTC. Wire `preflight-deploy.sh` into `scripts/release.sh` so the gate is mechanically enforced.
...
- **ADR-0039: 5 prompteval FAILs — all 4 baselines needed (ACTIVE)**: `prompteval check .` returns 5 FAILs: no baseline for `thread-opening-frame`, `codex-task-prompt`, `offline-synthesis-prompt`; `review-prompt` baseline stale (spec/executor/adapter drifted after bc71c3c) and not a release run. `enforce: false` is correctly set. Tick hit rate limit before baseline completed (2nd consecutive window — structural timing collision). Fix: attended session must run `prompteval run --id <id> --no-cache --release --update-baseline` × 4, then flip `enforce: true`, commit, deploy. **CRITICAL: `scripts/release.sh` does NOT call `preflight-deploy.sh` — the deploy gate is policy-only, not mechanically enforced. Wire preflight-deploy.sh into release.sh.**
- **Deploy gap: 3 post-release commits not live (ACTIVE)**: 479bd4c ("Verify either emitted or explicitly empty projection state"), eec71b2 ("Assert blocked research in browser acceptance"), 2a6d684 ("Label projection counts as contract integrity") — all committed after `20260712T201337Z-c28fd43` release. Not live. Will deploy once prompteval gate is unblocked.
...
1. This file.
2. **`prompteval check .`** — returns 5 FAILs (3 no baseline, review-prompt stale, none --release). Run `prompteval run --id <id> --no-cache --release --update-baseline` × 4 in an ATTENDED session (tick sessions hit rate limit before completing). Then flip `enforce: true`, commit uncommitted prompteval/src/prompts/ files, wire preflight-deploy.sh into release.sh, then deploy.
