...
**Last updated**: 2026-07-12T20:12Z — **owner observatory corrected after integration review; production remains on restored `21f7f34` pending final immutable release**. The consumer now validates the exact Synaplex public v1 snake_case contract and declared canonical digest. It reports only typed `research` / `findings` / `mechanisms` counts and explicit Decision/Evidence provenance; Claims, frozen gates, invariants, pod reuse, owner authority, handoff pressure, and public/private coherence remain `unknown` until typed sources exist. Unrelated CURRENT_STATE/handoff movement is never projection drift. Next/React migration remains staged at the patched 15.5.18/19.1.7 line. ADR-0039 prompt-eval work remains **preserved uncommitted and NOT trustworthy** — none of its contaminated baselines are acceptance evidence.

---

## Deployed / running state
- **Pending deploy in this change**: immutable release of the corrected ADR-0046 owner observatory after commit. The first Next 15 smoke exposed mutable dependency symlinks and rolled back into an incompatible crash; service was recovered to `20260712T124537Z-21f7f34-restored` with isolated Next 14.2.35 dependencies. Release assembly now keys full build/runtime dependencies from the staged lockfile before build, and rollback must prove service active plus `/login=200`. Cross-version release/rollback regression passes. Local/public authenticated browser evidence must still be recorded after the final release swap.
