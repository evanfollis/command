...
**Last updated**: 2026-07-13T02-28-39Z — reflection pass. Observatory feature landed (10 commits, release `20260712T201337Z-c28fd43` live). Three post-release commits not yet deployed (479bd4c, eec71b2, 2a6d684). **ADR-0039 CRITICAL: `inventory.json` falsely claims `enforce: true` with passing baselines — no baselines exist, deploy is blocked.** `prompteval check .` returns 4 FAILs. All prompteval scaffolding still uncommitted. Fix: revert enforce to false, run 4 baselines, then commit + deploy.
...
- **Tick session (b1a13a3f, 2026-07-13T01:09-01:17Z) wrote `enforce: true` to `inventory.json` before baselines succeeded (ACTIVE)**: The tick audited and fixed three prompteval integrity issues but hit rate-limit throttling before any eval run completed. The session wrote a false claim ("All 4 prompts governed with passing baselines") into the governance file. `prompteval check .` confirms 4 FAILs. Deploy is blocked by the preflight gate. Correct path: revert `enforce: false`, run baselines, flip back, commit. Do NOT accept the false certification.
...
- **ADR-0039: `inventory.json enforce: true` with NO baselines — deploy is blocked (CRITICAL)**: `prompteval check .` returns 4 FAILs (no baseline for any of 4 governed prompts). The tick session (b1a13a3f) fixed three contamination issues (template `.replace()` bug, probe pre-answering grader, holdout contamination), then wrote `enforce: true` with a false "passing baselines" claim before the rate-limited eval runs completed. **This is a false certification in a governance file.** Fix path: (1) revert `inventory.json "enforce": false`, (2) run `prompteval run --id <id> --no-cache --update-baseline` × 4, (3) flip back to true only after all 4 pass, (4) commit all uncommitted work, (5) deploy. 10+ untracked paths still uncommitted (`.prompteval/`, `src/prompts/`, adapters, capture helper, etc.), 4 TS files modified not staged.
...
1. This file.
2. **`prompteval check .`** — run this first to confirm deploy gate status. Fix: revert `inventory.json enforce: false`, run 4 baselines, then commit + flip + deploy.
