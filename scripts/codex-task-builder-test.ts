...
assert.match(complex, /keep signing and authentication credentials runtime-only/)
assert.match(complex, /non-secret exact canary literals and relevant encodings/)
assert.match(complex, /authenticated outcome checks against the intended release identity/)
assert.match(complex, /atomically restore a distinct pre-existing immutable release/)
assert.match(complex, /retain the failed release and failure receipt/)
