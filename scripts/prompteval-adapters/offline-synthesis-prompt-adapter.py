#!/usr/bin/env python3
"""prompteval `command` executor adapter for offline-synthesis-prompt.
...
def main() -> int:
    ...
    rendered = render_via_runtime("offline-synthesis-prompt", payload["prompt_text"], payload["input"])
    return print_result(lambda: run_prompt(rendered, model, payload, "offline-synthesis-prompt"))
